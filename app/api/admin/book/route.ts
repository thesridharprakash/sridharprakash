...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Unable to fetch booking content." }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: getAdminStorageWriteErrorMessage(error, "Booking content") || "Unable to save booking content." },
      { status: 500 }
    );
  }
}
